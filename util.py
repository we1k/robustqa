...
class CLFDataset(Dataset):
    ...
    # 0 for indomain, 1 for ood
    def __init__(self, in_encodings, out_encodings):
        self.in_encodings = in_encodings
        self.out_encodings = out_encodings
        self.keys = ['input_ids', 'attention_mask']

        self.encodings = in_encodings
        for key in self.keys:
          self.encodings[key] += self.out_encodings[key]

        self.in_num = len(list(self.in_encodings['input_ids']))
        self.out_num = len(list(self.out_encodings['input_ids']))
        self.label = torch.cat((torch.zeros(self.in_num), torch.ones(self.out_num)), -1).long()

    def __getitem__(self, idx):
        return {key : torch.tensor(self.encodings[key][idx]) for key in self.keys}, self.label[idx]
